            ...
            <div class="quiz__start--body">
              <div class="quiz__start--info">
        		...
        		<?if ($arResult["TEST"]["TIME_LIMIT"] > 0):?>
                  <strong><?=$arResult["TEST"]["TIME_LIMIT"]?> <?=GetMessage("LEARNING_TEST_TIME_LIMIT_MIN")?></strong>
                  <p><?=GetMessage("LEARNING_TEST_TIME_LIMIT")?>: </p>
				<?else:?>
                  <strong><?=GetMessage("LEARNING_TEST_TIME_LIMIT_UNLIMITED")?></strong>
                  <p><?=GetMessage("LEARNING_TEST_TIME_LIMIT")?>: </p>
				<?endif?>
                </div>
				...
				<?if ($arResult["TEST"]["ATTEMPT_LIMIT"] > 0):?>
                  <strong><?=$arResult["TEST"]["ATTEMPT_LIMIT"]?></strong>
                  <p><?=GetMessage("LEARNING_TEST_ATTEMPT_LIMIT")?>: </p>
				<?else:?>
                  <strong><?=GetMessage("LEARNING_TEST_ATTEMPT_UNLIMITED")?></strong>
                  <p><?=GetMessage("LEARNING_TEST_ATTEMPT_LIMIT")?>: </p>
				<?endif?>
...
<!--     <div class="quiz__modal--overlay">
      <div class="quiz__modal">
        ...
        <div class="quiz__modal--wrapper active">
				<div class="quiz__modal--header"><span><?=GetMessage("LEARNING_QUESTION_TITLE")?> <?=$arResult["NAV"]["PAGE_NUMBER"]?> / <?=$arResult["NAV"]["PAGE_COUNT"]?></span>
		            <h3><?=$arResult["QUESTION"]["NAME"]?></h3>
		            <?if ($arResult["QUESTION"]["QUESTION_TYPE"] == "M"):?>
		            	<?if ($arResult["QUESTION"]["FILE"] !== false):?>
		            		<img width="820" height="300" src="<?=$arResult["QUESTION"]["FILE"]["SRC"]?>">
		            	<?endif?>
		            <?endif?>
		        </div>
		          <div class="quiz__modal--body">
		          	<form name="learn_test_answer" class="quiz__form" action="<?=$arResult["ACTION_PAGE"]?>" method="post">
						<?=bitrix_sessid_post()?>
						...
						<?else:?>
							<?if ($arResult["QUESTION"]["QUESTION_TYPE"] == "M"):?><fieldset class="quiz__form--checkbox"><?elseif ($arResult["QUESTION"]["QUESTION_TYPE"] == "S"):?><fieldset class="quiz__form--image"><?endif?>
